...
#### Создание локальной копии чьего-либо репозитория
...
Результатом таких действий мы получим полную копию удалённого репозитория, он будет находится в папке с названием скачанного репозитория, в приведённом примере - `Srmnar3`
...
При работе в комманде обычно нельзя произвести изменения в общем репозитории на прямую (если не достаточно для этого прав), необходимо пройти соглавсование. Для этого на веб-сервисе [GitHub] существует такая процедура, называемая - *Pull Request*.

Для его осуществления потребуется аккаунт на [GitHub] (регистрация бесплатная и требует указания лишь имени пользователя и электронной почты).

Вот сам процесс в кратце.

1. Форкните проект.

    Необходимо нажать на сайте [GitHub] кнопку `Fork` на странице выбранного репозитория.
    Это создаст его копию репозитория в нашем аккаунте к которому у нас есть доступ на изменение.
...
6. Убедитесь, что проект не постардал после изменений.
