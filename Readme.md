...
Для того, что бы удалить ветку необходимо использовать команду *git branch -d*. Используется она следующим образом: в папке с репозиторием *git branch -d <название ветки>*. Еще есть команда *git branch -D* для удаление ветки принудительно без сохранений. Используется она в папке с репозиторием *git branch -D <название ветки>*.
